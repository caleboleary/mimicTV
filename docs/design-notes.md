...
- 2026-09-06 (service): all 22 rows have live tests. `apps/server`: JSON state under `data/`, ffprobe scanning with progress, publish planner with per-channel checkpoints (replay old rules to the next boundary, continue with new), atomic day files, XMLTV, lineup + channel.json from Next's real schemas, scheduled top-ups, live-break resolver. Setup page. Storage stays JSON; SQLite (built into Node 24) is the plan for play history later.
