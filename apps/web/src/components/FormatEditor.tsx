...
export default function FormatEditor({ clock, onChange: set }: Props) {
  ...
  const active = matchingPreset(clock);
  const noPad = clock.pad.toMinutes === 0;
  const between = clock.breaks.betweenPrograms ?? 1;

  return (
    <div className="recipe" style={{ gap: 12 }}>
      <div>
        <div className="presets">
          {PRESETS.map((p) => (
            <button key={p.id} type="button" className={`preset${active?.id === p.id ? ' on' : ''}`} title={p.hint} onClick={() => set((c) => applyPreset(c, p))}>
              <b>{p.label}</b>
            </button>
          ))}
          {!active && <div className="preset custom on"><b>Custom</b></div>}
        </div>
        <p className="preset-hint">{active ? active.hint : 'A custom format. Pick a preset to start over, or fine-tune below.'}</p>
      </div>

      <Disclosure label="Customize" openLabel="Customize">
        <div className="recipe" style={{ gap: 14 }}>
          <div>
            <h3 style={{ marginBottom: 8 }}>Slot</h3>
            <div className="form-grid">
              <label className="field">Content per slot (min)
              ...
              <label className="field">Give or take (min)
              ...
              <label className="field">Slots end on
                <select value={clock.pad.toMinutes} onChange={(e) => set((c) => ({ ...c, pad: { ...c.pad, toMinutes: Number(e.target.value) } }))}>
                  <option value={0}>whenever content ends (no padding)</option>
                  {[15, 30, 60].map((m) => <option key={m} value={m}>the next {m === 60 ? 'hour' : `:${m}`}</option>)}
                </select>
              </label>
          ...
          <div>
            ...
            <div className="form-grid">
              ...
              {clock.program.allowMultiple && (
                <label className="field">Between stacked programs
                  <select value={between} onChange={(e) => set((c) => ({ ...c, breaks: { ...c.breaks, betweenPrograms: Number(e.target.value) } }))}>
                    <option value={1}>a break after every program</option>
                    {[2, 3, 4, 5, 6].map((n) => <option key={n} value={n}>a break after every {n} programs</option>)}
                    <option value={0}>no breaks between programs</option>
                  </select>
                </label>
              )}
              {!noPad && (
                <>
                  <label className="check field">
                    ...
                    Make all breaks the same length
                  </label>
                  {!clock.breaks.equalize && (
                    <label className="field">Mid-roll length (sec)
                      <input type="number" step={5} value={Math.round(clock.breaks.midTargetMs / SEC)} onChange={(e) => set((c) => ({ ...c, breaks: { ...c.breaks, midTargetMs: Number(e.target.value) * SEC } }))} />
                    </label>
                  )}
              ...
              {!noPad && (
                <label className="check field">
                  ...
                  Pick ads at playback (ErsatzTV Next asks mimicTV during each break)
                </label>
              )}
            ...
            {clock.breaks.live && <p className="muted small" style={{ margin: '8px 0 0' }}>The preview still shows likely ads. In the written files each break is a placeholder ErsatzTV Next resolves live, so set the URL ErsatzTV Next can reach mimicTV on under Setup.</p>}
          </div>
