...
export const PRESETS: FormatPreset[] = [
  { id: 'sitcom', label: 'Half-hour show', hint: '22 min of content padded to :30. Breaks at chapters, else every 8 min. IDs near :00/:30.',
    shape: { program: { targetMs: 22 * MIN, toleranceMs: 4 * MIN, allowMultiple: true }, breaks: { atChapters: true, fallback: { mode: 'interval', everyMs: 8 * MIN }, betweenPrograms: 1, equalize: true, midTargetMs: 2 * MIN, maxItems: 0 }, pad: { toMinutes: 30 }, networkId: ids } },
  { id: 'shorts', label: 'Cartoon shorts', hint: 'Two 11-min shorts fill a half hour, a short break between them, up to 4 ads per break.',
    shape: { program: { targetMs: 22 * MIN, toleranceMs: 3 * MIN, allowMultiple: true }, breaks: { atChapters: true, fallback: { mode: 'none' }, betweenPrograms: 1, equalize: true, midTargetMs: 90 * SEC, maxItems: 4 }, pad: { toMinutes: 30 }, networkId: ids } },
  { id: 'drama', label: 'Hour drama', hint: '44 min of content padded to the hour. Breaks at chapters, else every 11 min.',
    shape: { program: { targetMs: 44 * MIN, toleranceMs: 6 * MIN, allowMultiple: false }, breaks: { atChapters: true, fallback: { mode: 'interval', everyMs: 11 * MIN }, betweenPrograms: 1, equalize: true, midTargetMs: 150 * SEC, maxItems: 0 }, pad: { toMinutes: 30 }, networkId: ids } },
  { id: 'movies', label: 'Movies', hint: 'One film per slot, padded to the next :30. A break every 25 min unless the file has chapters.',
    shape: { program: { targetMs: 100 * MIN, toleranceMs: 30 * MIN, allowMultiple: false }, breaks: { atChapters: true, fallback: { mode: 'interval', everyMs: 25 * MIN }, betweenPrograms: 1, equalize: true, midTargetMs: 3 * MIN, maxItems: 0 }, pad: { toMinutes: 30 }, networkId: ids } },
  { id: 'binge', label: 'Back to back', hint: 'No ads, no padding, no IDs. Each episode starts when the last one ends, like a streaming queue.',
    shape: { program: { targetMs: 22 * MIN, toleranceMs: 4 * MIN, allowMultiple: false }, breaks: { atChapters: false, fallback: { mode: 'none' }, betweenPrograms: 1, equalize: true, midTargetMs: 0, maxItems: 0 }, pad: { toMinutes: 0 }, networkId: { enabled: false } } },
  { id: 'videos', label: 'Music videos', hint: 'Clips stack to fill a half hour with a short break after every 4.',
    shape: { program: { targetMs: 27 * MIN, toleranceMs: 3 * MIN, allowMultiple: true }, breaks: { atChapters: false, fallback: { mode: 'none' }, betweenPrograms: 4, equalize: true, midTargetMs: 60 * SEC, maxItems: 3 }, pad: { toMinutes: 30 }, networkId: ids } },
];
